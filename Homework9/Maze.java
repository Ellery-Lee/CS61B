...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    //give the walls index
    if (vert > 1) {                                
    	for (j = 0; j < vert - 1; j++) {
    		for (i = 0; i < horiz; i++) {
				walls[index] = i + j * horiz;
				index++;
			}
    	}
	}
    ...
    for (i = 0; i < numWalls; i++) {
		index = walls[i];
		if (index >= 0) {
			int cell1 = maze.find(index);
			int cell2 = maze.find(index + horiz);
			if (cell1 != cell2) {
				maze.union(cell1, cell2);
				hWalls[index%horiz][index/horiz] = false;
			}
		}else {
			int cell1 = maze.find(-index - 1);
			int cell2 = maze.find(-index);
			if (cell1 != cell2) {
				maze.union(cell1, cell2);
				vWalls[(-index - 1)%horiz][(-index - 1)/horiz] = false;
			}
